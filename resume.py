...
from tkinter import filedialog, Tk, Label, Button, StringVar, IntVar, Entry, Frame, Toplevel, Text, Scrollbar, END
...
class Application(Frame):
    ...
    def create_widgets(self):
        self.start_chunk = StringVar()
        self.end_chunk = StringVar()
        self.start_chunk.set("0")
        self.end_chunk.set("-1")

        self.start_label = Label(self, text="Start Chunk")
        self.start_label.pack(side="top")
        self.start_entry = Entry(self, textvariable=self.start_chunk)
        self.start_entry.pack(side="top")

        self.end_label = Label(self, text="End Chunk")
        self.end_label.pack(side="top")
        self.end_entry = Entry(self, textvariable=self.end_chunk)
    ...
    def start_processing(self):
        start_chunk = int(self.start_chunk.get())
        end_chunk = int(self.end_chunk.get())
        Thread(target=self.process_chunks, args=(start_chunk, end_chunk)).start()

    def process_chunks(self, start_chunk, end_chunk):
